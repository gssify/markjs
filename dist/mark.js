...
var Mark = function () {
  ...
  _createClass(Mark, [{
    ...
    key: 'cursorInit',
    value: function cursorInit() {
      var _this = this;

      this._cursors = [1, 2, 3, 4].map(function (item) {
        var cursor = _this.draw.rect(10, 10);
        cursor.data('action', ACTION_RESIZE).fill({ color: MAIN_COLOR, opacity: 0.5 }).style('cursor', 'pointer').hide();
        return cursor;
      });
    }
  }, {
    ...
    value: function marksInit() {
      ...
      this.marks.map(function (mark) {
        var polygon = _this2.paintPolygon(mark.points, ACTION_DELETE);
        polygon.data('data', mark.data);

        mark.__instance = polygon;
        mark = Object.assign(mark, _this2.dataFormat(mark));
        if (mark.text) {
          mark.__text = _this2.paintText(mark.text, mark.points);
        }
    ...
    key: 'paintText',
    value: function paintText(text, points) {
      if (!points instanceof Array) return;
      var point = getCentroid(points);
      var svgText = this.draw.text(text);
      var textRect = this.draw.rect(0, 0);
      ...
      textRect.back().move(svgText.x() - OFFSET / 2, svgText.y() - OFFSET / 2).size(w + OFFSET, h + OFFSET).fill({
        color: MAIN_COLOR,
        opacity: 0.4
      });
    ...
    key: 'resizeMark',
    value: function resizeMark() {
      var _this3 = this;

      // refresh polygon and cursor
      var cursor = this._cursors.find(function (cursor) {
        return cursor === _this3._handle;
      });
      var index = this._cursors.findIndex(function (cursor) {
        return cursor === _this3._handle;
      });
      var points = this._points.slice(0).map(function (point) {
    ...
    value: function onDblClick(evt) {
      var instance = evt.target.instance;
    ...
    value: function onKeydown(evt) {
      ...
      if (evt.keyCode === 83 && evt.ctrlKey) {
        ...
        this.beforeAdd(function (status, data) {
          ...
          if (mark.text) {
            mark.__text = _this4.paintText(mark.text, mark.points);
          }

          _this4._polygon.data('data', data);
          _this4.marks.push(mark);
    ...
    value: function onMouseDown(evt) {
      var svgRect = this.el.getBoundingClientRect();
    ...
    key: 'onMouseDownHook',
    value: function onMouseDownHook() {
    ...
    key: 'onMouseMoveHook',
    value: function onMouseMoveHook() {
      if (this._action === ACTION_RESIZE) {
        this.resizeMark();
      }
    ...
    value: function onMouseUpHook() {
      ...
      if (this._action === ACTION_ADD) {
        ...
        var points = this.getRectPoints();
        points.forEach(function (point, index) {
          _this5._cursors[index].move(point[0] - 5, point[1] - 5).front().show();
        });
        this._resize = true;
      }
    ...
    key: 'getRectPoints',
    value: function getRectPoints() {
      // get four end point
      var points = [[this._startLeft, this._startTop], [this._startLeft + this._disX, this._startTop], [this._startLeft + this._disX, this._startTop + this._disY], [this._startLeft, this._startTop + this._disY]];
      return points;
    }
  }, {
    key: 'getMarks',
